...
const JSONImportForm = ({ groupId, navigation }: { groupId: string; navigation: any }) => {
    ...
    const isValid =
        !!text && Array.isArray(parsedText) && parsedText.length > 0 && parsedText[0]?.question && parsedText[0]?.answer
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            >
                {state.loading
                    ? "Generating..."
                    : state.data?.generateAIFlashcards?.length === 0
                      ? "Generate Flashcards"
                            ...
                            renderItem={({ item: flashCard, index }) => (
                                <TouchableOpacity
                                    ...
                                    style={{
                                        marginVertical: 4,
                                        padding: 12,
                                        backgroundColor: selectedCards.has(index)
                                            ? lowOpacity(Colors.secondary, 0.2)
                                            : Colors.primary_lighter,
                                        borderRadius: 6,
                                        borderWidth: selectedCards.has(index) ? 1 : 1,
                                ...
                                >
                                    <View style={{ flexDirection: "row", alignItems: "flex-start" }}>
                                        <View style={{ flex: 1 }}>
                                            ...
                                            <Text style={{ color: "#ccc", fontSize: 12, marginBottom: 3 }}>
                                                {flashCard.answer}
                                            </Text>
