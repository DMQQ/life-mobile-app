...
export const useFlashCards = (groupId?: string): UseFlashCardsReturn => {
    ...
    const [createFlashCardMutation] = useMutation<CreateFlashCardMutationResponse, { input: CreateFlashCardInput }>(
        CREATE_FLASHCARD,
        {
            refetchQueries: [
    ...
    const [updateFlashCardMutation] = useMutation<UpdateFlashCardMutationResponse, { input: UpdateFlashCardInput }>(
        UPDATE_FLASHCARD,
    )

    const [reviewFlashCardMutation] = useMutation<
        ReviewFlashCardMutationResponse,
        { input: { id: string; isCorrect: boolean } }
    >(REVIEW_FLASHCARD)
