...
const Menu = ({ isVisible, navigation, setIsVisible }: { isVisible: boolean; navigation: any; setIsVisible: any }) =>
  isVisible ? (
    <AnimatedPressable entering={FadeIn} exiting={FadeOut} onPress={() => setIsVisible(false)} style={styles.menu}>
      <View
        // layout={Layout}
      ...
      >
        ...
        />
        <IconButton icon="search" onPress={() => {}} color={Color(Colors.secondary).darken(0.2).string()} delay={100} />
        <IconButton icon="options" onPress={() => {}} color={Color(Colors.secondary).darken(0.4).string()} delay={isVisible ? 150 : 50} />
        <IconButton
          icon="close"
