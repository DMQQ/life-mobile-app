...
const ClockTimer = (props: ClockTimerProps) => {
  ...
  return (
    <View>
      <Ripple onPress={onStopTimer} onLongPress={() => setShowSettings((p) => !p)}>
        <Svg width={radius * 2} height={radius * 2}>
          <Circle cx={radius} cy={radius} r={radius - strokeWidth / 2} fill="none" stroke={Colors.primary} strokeWidth={strokeWidth} />
          <AnimatedCircle
            strokeLinecap={"round"}
            cx={radius}
            cy={radius}
          ...
          />
          <Text fontSize={60} x={"50%"} y={"50%"} textAnchor="middle" fill={Colors.secondary} fontWeight={"bold"} letterSpacing={0.5}>
            {!isActive ? (timer === 0 ? "Time's up" : "Paused") : `${time(timer)}`}
          </Text>

          {props?.text && (
            <Text fontSize={18} x={"50%"} y={"60%"} textAnchor="middle" fill={"gray"} fontWeight={"bold"} letterSpacing={0.5}>
              {props.text}
            </Text>
          )}
        </Svg>
